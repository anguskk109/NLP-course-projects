...
def build_data_train_dev_split(opts: argparse.Namespace):
    ...
    for prefix_path, prefixes in (
        ...
        with a2_utils.smart_open(prefix_path, "wt") as open_file:
            open_file.write("\n".join(prefixes))
            open_file.write("\n")
...
def train(opts: argparse.Namespace):
    ...
    with a2_utils.smart_open(opts.train_prefixes, "rt") as open_file:
    ...
    del train_prefixes
    with a2_utils.smart_open(opts.dev_prefixes, "rt") as open_file:
...
def interact(opts: argparse.Namespace):
    ...
    dataset = a2_dataloader.HansardEmptyDataset(
    ...
    with a2_utils.smart_open(opts.model_path, "rb") as model_file:
