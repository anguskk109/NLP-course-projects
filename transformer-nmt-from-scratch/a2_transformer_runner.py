...
class TransformerRunner:
    ...
    def train_input_target_split(
        ...
        targets = target_tokens[:, 1:].contiguous().view(-1)
    ...
    def train_for_epoch(
        ...
        for step, (source, source_lens, targets) in enumerate(dataloader):
            ...
            loss = criterion(logits.view(-1, logits.size(-1)), criterion_targets.view(-1))
    ...
    def translate(self, input_sentence):
        ...
        # assert False, "Fill me"
        source_tokens = torch.LongTensor([self.dataset.source_word2id.get(token, self.dataset.source_unk_id) for token in tokens]).unsqueeze(0)
        ...
        # assert False, "Fill me"
        output_tokens = hypotheses[0].tolist()
        decoded_ouput = [self.dataset.target_id2word[token_id] for token_id in output_tokens
                         if token_id != eos_idx and token_id != pad_idx]
        
        return " ".join(decoded_ouput)
    ...
    def compute_batch_total_bleu(
        ...
        """
        # assert False, "Fill me"
        batch_bleu_scores = [0.0 for _ in n_gram_levels]
        batch_size = target_y_ref.size(0)

        for i in range(batch_size):

            ref_seq = target_y_ref[i].tolist()
            ref_seq = [token for token in ref_seq if token not in {sos_idx, eos_idx, pad_idx}]

            can_seq = target_y_cand[i].tolist()
            can_seq = [token for token in can_seq if token not in {sos_idx, eos_idx, pad_idx}]

            for x, n in enumerate(n_gram_levels):
                batch_bleu_scores[x] += bleu_score_func(ref_seq, can_seq, n)

        return batch_bleu_scores, batch_size
